...
1. Data quality checks for missing values and outliers in excel. Can outliers or missing values be explained by an event or are there mistakes? Flag and comment in excel, go back to data source to check when needed.
...
```
  ...
  3.conversions (MMP only - whitsundays and central)
    calculate NOX, convert units for NOX, PN, PP
    
  4. LOR/LOD check (mainly SIP) 
    transform values too low for detection, check limits and calculations (LOR * 0.5)
  

## Indicator tables

5) use sites and guidelines table to get annual mean or median for each site

6) calc unstandardised indicator score - if the log of GV value mean or median / real value is less than 1, =1, or 2 - basically calculating whether its higher or lower than guideline value

7) Avg of indicators to form indicator category (need to have 60% of indicators to grade)

8) Indicator zone level (avg of sties)

9) standardised score (join indicator zone level score to WQ index to determine Condition and resulting equation to calc GBR range)

10) avg of nutrients, chl-a, clarity, and pesticides to get WQ grade, need 60% (3/4) to write results.