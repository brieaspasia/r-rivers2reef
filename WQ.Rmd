...
library(gridExtra) #grid prints for ggplot
library(psych) #describe function for summary table
...
```{r whit-scores}
#*** Still need to figure out nObsv col and direction of failure
# 3) Stats (site (name + code), indicator, n-samples, mean, min, 25%, median, 75%, max, direction of failure, stat, GV)

#data wide to long
t <- nutrients_whit %>%
  gather(Indicator, Value, Secchi:PP) %>%
  select(-Date, -zone, -supplier, -Season) %>%
...
#   filter(Code %in% whit) %>%
#   rename(Chla = 'Chl-a',
#          Turbidity = 'Turbidity_annual') %>%
#   gather(Indicator, GV, NOx:Turbidity) %>%
#     select(Code, Name, Indicator, GV) %>%
#   mutate(Stat = ifelse(Indicator == "Secchi", "mean", "median"),
#          Failure = ifelse(Indicator == "Secchi", "Low", "High"))
