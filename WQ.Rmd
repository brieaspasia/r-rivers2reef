...
whit_standardised <- whit_stats %>%
        ...
        mutate(standardised = ifelse((log((GV/Stat_val),2))<=-1,-1,(ifelse((log((GV/Stat_val),2))>=1,1,(log((GV/Stat_val),2)))))) %>%
        select(-GV, -Stat_val) %>%
        pivot_wider(names_from = Indicator, 
                    values_from = Standardised)

whit_categories <- whit_standardised %>%
  mutate(Nutrients = mean(NOx:PP),
         Chla = Chla,
         Clarity = mean(TSS:Secchi)) %>%
...
"#00B050", "#92D050", "#FFFF00", "#FFC000", "#FF0000"

colours <- col_bin(
  palette=c("#00B050", "#92D050", "#FFFF00", "#FFC000", "#FF0000"),
  domain=c(81:100, 61:80, 41:60, 21:40, 0:20),
  bins = 5,
  pretty = TRUE,
  na.color = "#808080",
  alpha = FALSE,
...
color.picker <- function(z){
  if(z <= 81){return("#00B050")}
  else if( z > 60 & z <= 80){return("#92D050")}
  else {return("#FFFF00")}
}

text.cols <- sapply(c(whit_zone_grade), color.picker)
...
# #example from WH1 NOx 19/20
# ifelse((log((1/.77),2))<=-1,-1,(ifelse((log((1/.77),2))>=1,1,(log((1/.77),2)))))
